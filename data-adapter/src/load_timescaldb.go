...
func loadDataTimeScaleDB(timeseries map[uuid.UUID][]map[string]interface{}) {
	...
	for uuid, values := range timeseries {
		...
		switch value := values[0]["Value"].(type) {
		...
		queryTimeScale(createTableQuery, "postgres", "password", "5432", "postgres")
		...
		queryMultipleTimeScaleSameQuery(insertQuery, parameters, "postgres", "password", "5432", "postgres")
	}
